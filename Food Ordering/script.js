...
function generateCard(data) {
  ...
  locimg.src =
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTSIzMUB0QDcMBSfnZoQQexkXL4mL7NhR2ymvlEliDnM9It1yuT";
...
function sortBy(value) {
  ...
  if (value === "Name") {
    ...
    list.sort((i, j) =>
      value === "ETA" ? i[value] - j[value] : j[value] - i[value]
    );
  }
  generateView(list);
}
...
function filterBy(value) {
  ...
  if (value !== "All") {
    list = list.filter(({ tags }) => tags.includes(value));
  }
  generateView(list);
}
